...
function getBackgoundImage (){
  const allPlayerCards = Array.from(document.querySelectorAll(".player-card"));
...
(async () => {
    ...
    response.forEach((res) => {
        if (i < allPlayerCards.length) { 
            allPlayerCards[i].style.backgroundImage = `linear-gradient( #000000c5, #000000d9 ), url(${res})`;
        }
        i++;
    });
});
...
playersDropdownList.addEventListener("change", (e)=>{
  ...
  switch (e.target.value) {
    case "nickname" :
    setPlayerCards(players.filter(({nickname})=> nickname != null));
    break;
    case "forward" : 
